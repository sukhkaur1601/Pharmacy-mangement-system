...
public class PurchaseOrderService {
	...
	public void createPurchaseOrder(PurchaseOrder purchaseOrder) {
		...
		int poId=  purchaseOrderRepository.save(purchaseOrder).getPoId();
		
		  
		  double sum = 0; int totalQuantity=0;
		  ...
		  purchaseOrder.setSubTotal(sum); purchaseOrder.setTax(sum*0.15);
	...
	// -----------------------Method for get all purchase order product
	// list--------------------------------
	public List<PurchaseOrder> getAllProducts() {
		return purchaseOrderRepository.findAll();
	}

	// -----------------------Method for get order by
	// id----------------------------------
	public Optional<PurchaseOrder> getallOrderById(int id) {
		return purchaseOrderRepository.findById(id);
	}

	// -------------------Method for update
	// order---------------------------------------
	public PurchaseOrder updatePurchaseOrder(int id, PurchaseOrder purchase) {
	...
	public Product getProductDetails(int productId) {
		...
		String uri = "http://" + instanceInfo.getIPAddr() + ":" + instanceInfo.getPort() + "/product/searchproduct/"
				+ productId;
		System.out.println("URL" + uri);

		// final String uri =
		// "http://localhost:8762/product-managment-service/product/searchproduct/"+productId;
	...
	// ---------------------------------Getting Products according to Order
	// Date------------------------------
	public List<PurchaseOrder> getReport(LocalDate from, LocalDate to) {
